...
/*   Updated: 2021/11/26 21:49:06 by pllucian         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	key_press(int key, t_data *data)
{
	...
	if (key == KEY_A)
	{
		data->plr_img_ptr = &data->plr_img_lbonus;
		move_left(data);
	}
	if (key == KEY_S)
		move_down(data);
	if (key == KEY_D)
	{
		data->plr_img_ptr = &data->plr_img_rbonus;
		move_right(data);
	}
	return (EXIT_SUCCESS);
}
...
void	init_textures(t_data *data)
{
	...
	init_texture(data, &data->plr_img_lbonus, data->map.plr_path_lbonus);
	init_texture(data, &data->plr_img_rbonus, data->map.plr_path_rbonus);
	init_texture(data, &data->enemy_img_lbonus, data->map.enemy_path_lbonus);
	init_texture(data, &data->enemy_img_rbonus, data->map.enemy_path_rbonus);
	data->plr_img_ptr = &data->plr_img_lbonus;
...
void	game(t_data *data)
{
	...
	data->map.res_y = data->map.height * 64 + 10;
